...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // DRY Sidebar Content Helper
  const SidebarContent = ({ onClose }: { onClose?: () => void }) => (
    <div className="flex flex-col justify-between h-full">
      <div className="flex flex-col">
        {/* Logo area */}
        <div className="px-6 py-6 sm:py-8 border-b border-slate-800/60 flex items-center justify-between gap-3">
          ...
          {onClose && (
            <button onClick={onClose} className="md:hidden p-1.5 text-slate-500 hover:text-white transition-colors rounded-lg bg-slate-900/50 hover:bg-slate-900 border border-slate-800">
        ...
        <nav className="p-4 flex flex-col gap-1.5 mt-6">
          ...
          <Link
            href="/outreach"
            onClick={onClose}
            className={`flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 group font-medium ${pathname === '/outreach' ? 'bg-teal-500/10 text-teal-400 border border-teal-500/20 shadow-[0_0_15px_-3px_rgba(20,184,166,0.1)]' : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900/50 border border-transparent'}`}
          >
            <svg className="w-5 h-5 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
  ...
  return (
    <div className="w-full min-h-screen flex flex-col md:flex-row relative">
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950/60 backdrop-blur-xl border-b border-slate-900/60 z-30 flex items-center justify-between px-4 sm:px-6">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-teal-500 to-cyan-400 flex items-center justify-center shadow-lg shadow-teal-500/20">
            <svg className="w-4.5 h-4.5 text-slate-950" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.011 19.851c-1.394 1.157-3.155 1.854-5.077 1.854-4.385 0-7.94-3.555-7.94-7.94 0-1.922.697-3.683 1.854-5.077M20.303 14.545c.677-1.423 1.061-3.02 1.061-4.707 0-6.075-4.925-11-11-11-1.687 0-3.284.384-4.707 1.061m14.551 14.551a9.76 9.76 0 0 1-5.696 2.24m5.696-2.24c1.55-1.55 2.502-3.693 2.502-6.06 0-2.368-.952-4.51-2.502-6.06m-2.24 12.12a9.76 9.76 0 0 1-2.24 5.696m2.24-5.696V12m-6.06-6.06a9.76 9.76 0 0 1 6.06-2.24m-6.06 2.24c-1.55 1.55-2.502 3.693-2.502 6.06 0 2.368.952 4.51 2.502 6.06m-3.82-12.12a9.76 9.76 0 0 1 2.24-5.696m-2.24 5.696v2.24" />
            </svg>
          </div>
          <span className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-300 text-base tracking-tight">
            Signal Hunter
          </span>
        </div>
        ...
        <button
          ...
          className="p-2 text-slate-400 hover:text-white transition-colors rounded-xl bg-slate-900/40 border border-slate-800/60 active:scale-95"
        >
      ...
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-slate-950/95 backdrop-blur-2xl border-r border-slate-800/60 z-50 flex flex-col justify-between transition-transform duration-300 ease-out md:hidden ${
      ...
      <aside className="fixed inset-y-0 left-0 w-64 bg-slate-950/60 backdrop-blur-xl border-r border-slate-800/60 md:flex flex-col justify-between z-20 hidden">
        <SidebarContent />
      </aside>

      {/* Main Content Area */}
      <main className="flex-1 md:pl-64 z-10 relative pt-16 md:pt-0 w-full min-w-0">
        <div className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto w-full min-h-screen flex flex-col justify-between">
