...
function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    <span className={`text-[11px] font-bold tracking-wider px-2.5 py-1 rounded-lg border ${color} inline-block`}>
      {score} · {label}
    </span>
  );
}
...
export default async function SignalsPage({
  ...
  searchParams: Promise<{ minScore?: string; status?: string; city?: string }>;
}) {
  ...
  const filtered = cityFilter
    ? signals?.filter((s: any) => s.companies?.city?.toLowerCase().includes(cityFilter.toLowerCase()))
    : signals;

  // Métricas resumen
  const total = signals?.length ?? 0;
  const toContact = signals?.filter((s: any) => s.severity_score >= 85).length ?? 0;
  const toReview = signals?.filter((s: any) => s.severity_score >= 65 && s.severity_score < 85).length ?? 0;

  return (
    <div className="space-y-8">
      {/* Title block */}
      <div>
        <div className="text-xs uppercase tracking-widest text-teal-400 font-extrabold mb-1">Panel de Radar</div>
        <h1 className="text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
          Señales Detectadas
        </h1>
        <p className="text-slate-400 text-sm mt-2">
          Monitoreo en tiempo real de heridas críticas y crisis de reputación en negocios locales de LATAM.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="relative group overflow-hidden bg-slate-950/40 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 transition-all duration-300 hover:border-slate-700/80">
          ...
          <div className="text-slate-500 text-xs font-bold uppercase tracking-wider">Total señales</div>
          <div className="text-4xl font-extrabold text-white mt-2 flex items-baseline gap-2">
            {total}
            <span className="text-xs text-slate-500 font-normal">activas</span>
          </div>
        ...
        <div className="relative group overflow-hidden bg-slate-950/40 backdrop-blur-xl border border-rose-950/40 rounded-2xl p-6 transition-all duration-300 hover:border-rose-900/60">
          ...
          <div className="text-4xl font-extrabold text-rose-400 mt-2 flex items-baseline gap-2">
            {toContact}
            <span className="text-xs text-rose-500/60 font-normal">listas</span>
          </div>
          <div className="w-full h-1 bg-slate-800 rounded-full mt-4 overflow-hidden">
            <div className="h-full bg-rose-500 rounded-full transition-all duration-500" style={{ width: `${total ? (toContact / total) * 100 : 0}%` }} />
          </div>
        </div>
        ...
        <div className="relative group overflow-hidden bg-slate-950/40 backdrop-blur-xl border border-amber-950/40 rounded-2xl p-6 transition-all duration-300 hover:border-amber-900/60">
          ...
          <div className="text-amber-400/80 text-xs font-bold uppercase tracking-wider">Para revisar (65-84)</div>
          <div className="text-4xl font-extrabold text-amber-400 mt-2 flex items-baseline gap-2">
            {toReview}
            <span className="text-xs text-amber-500/60 font-normal">en espera</span>
          </div>
          <div className="w-full h-1 bg-slate-800 rounded-full mt-4 overflow-hidden">
            <div className="h-full bg-amber-500 rounded-full transition-all duration-500" style={{ width: `${total ? (toReview / total) * 100 : 0}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-950/20 backdrop-blur-md border border-slate-800/60 rounded-2xl p-5">
        <form method="GET" className="flex flex-col sm:flex-row gap-4 items-end">
          <div className="flex-1 w-full space-y-1.5">
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Score mínimo</label>
            <input
              name="minScore"
              type="number"
              placeholder="0 - 100"
          ...
          <div className="flex-1 w-full space-y-1.5">
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Ciudad</label>
            <input
              name="city"
      ...
      {/* Tabla */}
      <div className="bg-slate-950/40 backdrop-blur-xl border border-slate-800/60 rounded-2xl overflow-hidden">
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-slate-800/80 text-slate-500 text-xs font-bold uppercase tracking-wider bg-slate-900/30">
                ...
                <th className="px-6 py-4">Descripción de la Señal</th>
                <th className="px-6 py-4 text-right">Detectada</th>
              </tr>
            </thead>
                ...
                <tr key={signal.id} className="hover:bg-slate-900/30 transition-all duration-200 group">
                    ...
                    <div className="flex flex-col">
                      ...
                      {signal.companies?.rating && (
                        <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-1">
                          ...
                          <span>({signal.companies.review_count} reseñas)</span>
                        </div>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right whitespace-nowrap">
                    <span className="text-slate-500 text-xs font-medium">
                      {new Date(signal.detected_at).toLocaleDateString('es', {
        ...
        {(filtered ?? []).length === 0 && (
          <div className="text-center text-slate-500 py-20 bg-slate-950/20">
            ...
            <p className="font-semibold text-slate-400">Sin señales detectadas</p>
            <p className="text-xs text-slate-600 mt-1">Intenta con otros filtros o corre el radar.</p>
          </div>
        )}
      </div>
    </div>
